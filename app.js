...
function removeGrid() {
    // Remove grid boundary layer
    if (ndviGridLayer && map.hasLayer(ndviGridLayer)) {
        map.removeLayer(ndviGridLayer);
        console.log("[GRID] Removed existing grid layer.");
    }
    ndviGridLayer = null;

    // Remove any currently visible labels
    gridLabels.forEach(label => {
        if (map.hasLayer(label)) {
            map.removeLayer(label);
        }
    });
    // Clear the array holding all label markers
    gridLabels = [];
    console.log("[GRID] Cleared grid labels array and removed visible labels.");

    // Remove event listeners if they were added specifically for this grid (optional, less common)
    // map.off('moveend', updateVisibleGridLabels);
    // map.off('zoomend', updateVisibleGridLabels);
}
...
function displayGridOnCheckbox() {
    ...
    if (!popupCheckbox.checked || !currentParcel || !currentParcel.layers || availableDates.length === 0) {
        ...
        // Ensure map event listeners for labels are off if we are not displaying grids
        map.off('moveend', updateVisibleGridLabels);
        map.off('zoomend', updateVisibleGridLabels);
        return;
    }
    ...
    try {
        ...
        if (parts.length === 3) {
            dateFormattedForFile = `${parts[1]}-${parts[0]}-${parts[2]}`; // Reassemble as MM-DD-YYYY
        } else {
    ...
    console.log("[GRID] Attempting to load grid GeoJSON from:", geojsonUrl);
    ...
    fetch(geojsonUrl)
        ...
        .then(data => {
            // Check context validity again after fetch
            ...
            console.log("[GRID] Grid GeoJSON loaded successfully. Creating layer and labels...");
            gridLabels = []; // Ensure gridLabels is empty before populating
                ...
                onEachFeature: function(feature, layer) {
                    ...
                    if (ndviValue !== undefined && elevation !== undefined && !isNaN(ndviValue) && !isNaN(elevation)) {
                        try {
                            ...
                            const center = layer.getBounds().getCenter(); // Get the center of the grid cell
                            const labelIcon = L.divIcon({
                                className: 'grid-label', // Use the CSS class
                                ...
                                iconAnchor: [18, 10] // Adjust anchor to center label
                            });
                            const label = L.marker(center, { icon: labelIcon });

                            // *** IMPORTANT CHANGE: DO NOT ADD TO MAP YET ***
                            // label.addTo(map); // <-- REMOVE THIS LINE

                            // Store the label marker in the array
            ...
            }).addTo(map); // Add the grid boundaries to the map

            console.log(`[GRID] Created ${gridLabels.length} label markers in memory.`);

            // *** Add map event listeners for showing/hiding labels based on view ***
            // Remove existing listeners first to prevent duplicates
            map.off('moveend', updateVisibleGridLabels);
            map.off('zoomend', updateVisibleGridLabels);
            // Add the listeners
            map.on('moveend', updateVisibleGridLabels);
            map.on('zoomend', updateVisibleGridLabels);

            // *** Initial call to display labels in the current view ***
            updateVisibleGridLabels();

            console.log(`[GRID] Grid boundaries added. Initial labels displayed for current view.`);

        }).catch(error => {
            console.error('[GRID] Error loading or processing GeoJSON grid:', error);
            removeGrid(); // Clean up on error
        });
}

/**
 * Updates which grid labels are currently displayed on the map based on the
 * current map viewport bounds.
 */
function updateVisibleGridLabels() {
    if (!map || gridLabels.length === 0 || !popupCheckbox.checked) {
        // console.log("[GRID LABELS] Update skipped: No map, no labels, or checkbox off.");
        return;
    }

    const bounds = map.getBounds(); // Get current map viewport bounds
    // console.log("[GRID LABELS] Updating visible labels for bounds:", bounds.toBBoxString()); // Uncomment for debugging
    ...
    gridLabels.forEach(label => {
        ...
        // Check if the label's position is within the current map bounds
        if (bounds.contains(latLng)) {
            // If it's in bounds and NOT currently on the map, add it
            if (!map.hasLayer(label)) {
                label.addTo(map);
                addedCount++;
            }
        } else {
            // If it's OUT of bounds and IS currently on the map, remove it
            if (map.hasLayer(label)) {
                map.removeLayer(label);
                removedCount++;
            }
...
async function initializeApp() {
    ...
    setupAddParcelForm(); // Initialize the form for managing parcels
     ...
     // Link popupCheckbox to displayGridOnCheckbox - this is already correct
     ...
     timeSlider.addEventListener("change", () => {
         // Also update grid display when date changes on a grid-enabled parcel
         if (currentParcel && currentParcel.csvAvailable) { // Assuming CSV available implies grid possible
              displayGridOnCheckbox(); // Re-run the display logic for the new date
         }
     });
     ...
     // *** IMPORTANT: Add event listeners for map pan/zoom to update *labels* ***
     // These listeners will be added/removed by displayGridOnCheckbox itself
     // map.on('moveend', updateVisibleGridLabels); // <-- REMOVE: Handled by displayGridOnCheckbox now
     // map.on('zoomend', updateVisibleGridLabels); // <-- REMOVE: Handled by displayGridOnCheckbox now
